...
function powerSet(str){
  ...
  var cache = {} // cache will help us keep track of past calculations
  cache[0] = [''];
  cache[1] = ['', str[0]]
  function getAllSets(k, string){
    if (k === 1){ return cache[k]} //base case
    else{
      // if past calculations is not in cache, simply calculate it
      if (cache[k - 1] === undefined){
        getAllSets(k - 1, string)
      }
      var clone = cache[k - 1] // clone is P(n-1) -- previous calculation

      // newSubSet takes from the clone variable and manifest the new set into it
      var newSubSet = clone.map( letter => letter.concat(string[k - 1]))
      // update the result variable that we defined
      result = result.concat(newSubSet)
      // the newly result will be cached into memory for future calculations
      cache[k] = result
    }
  }

  // if string length is 0 or 1, simply get it from cache
  // otherwise let's recursivly find the solution
  if (length === 0 || length === 1){
    return cache[length]
  } else{
    result = cache[1] // anything more than length of 2 will guarntee at least sets
    getAllSets(length, str)
  }
  return result;
}
